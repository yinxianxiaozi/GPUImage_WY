...
@implementation CCFilterHandler
...
- (void)setEffectFilter:(GPUImageFilter *)filter {
    ...
    if ([filter isKindOfClass:[CCGPUImageBaseFilter class]]) {
        ((CCGPUImageBaseFilter *)filter).beginTime = self.displayLink.timestamp;
    }
}
...
- (void)displayAction {
    if ([self.currentEffectFilter isKindOfClass:[CCGPUImageBaseFilter class]]) {
        CCGPUImageBaseFilter *filter = (CCGPUImageBaseFilter *)self.currentEffectFilter;
